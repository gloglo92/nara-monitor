...
- 매일 전일 데이터를 수집하여 Excel 저장 + 텔레그램 발송 + JSON 저장
"""
...
def fetch_all_pages(start_dt: str, end_dt: str) -> list[dict]:
    ...
    while True:
        ...
        try:
            ...
            logger.info(f"  page {page}: {len(row_list)}건 (누계 {len(all_items)}/{total_cnt})")
...
def filter_by_keywords(df: pd.DataFrame) -> pd.DataFrame:
    ...
    filtered.index += 1
    logger.info(f"키워드 필터링: {len(df)}건 → {len(filtered)}건")
    return filtered
...
def build_dataframe(items: list[dict]) -> pd.DataFrame:
    ...
    df["합계발주금액(원)"] = (
        pd.to_numeric(df["합계발주금액(원)"], errors="coerce").fillna(0).astype(int)
    )
...
def save_excel(df: pd.DataFrame, date_str: str) -> str:
    ...
    with pd.ExcelWriter(filepath, engine="openpyxl") as writer:
        df.to_excel(writer, sheet_name="기술용역_발주계획", index=True, index_label="순위")
...
def main():
    ...
    logger.info(f"▶ 발주계획 수집 시작: {date_str}")
