...
# ── 설계 용역 소액 강등 기준 ──────────────────────────────────
DESIGN_BUDGET_THRESHOLD = 100_000_000  # 1억 원 미만 → 검토


def get_budget_field(data_type: str) -> str:
    """데이터 타입에 따른 예산 필드명 반환"""
    if data_type == "사전규격":
        return "배정예산액(원)"
    return "합계발주금액(원)"
...
def parse_budget(value: str) -> int:
    """'1,007,910,000' 형태의 문자열을 정수로 변환. 파싱 실패 시 0 반환."""
    try:
...
def save_json_data(df: pd.DataFrame, date_str: str, data_type: str) -> None:
    ...
    # 설계 용역 1억 미만 → 검토 강등
    budget_field = get_budget_field(data_type)
    demoted = []
    confirmed_items = []
